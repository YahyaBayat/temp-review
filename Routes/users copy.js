...
//global varyables
let trialTestTime=3* 60 * 1000; // 50 minutes in milliseconds for trial testing
let chargeTime=4* 60 * 1000; // 10 minutes in milliseconds for charghing period
let deActivePeriodTime=2* 60 * 1000; // 50 minutes in milliseconds for deactive period
let tokenJWTtime='4m'
// =========================start internal coding===========================
let userInfo = userStore.getUserInfo();
RemainActiveTimeChecking(userInfo.id);
// ==============================================
// ***************routings***************
//**************login Page****************
    ...
    if(foundUser){
      ...
      if(userInfo.port !== userInfo.port){ // check Perimission Port:
        ...
        req.logIn(user, (err) => {
          ...
          return res.json({LoginAuthentication: true,userKey1:userInfo.key1,msg:userInfo.port,RemainChargTime:RemainChargTime,chargStatus:chargStatus,userLevel:userInfo.userLevel, token:token,chargingDebt:chargingDebt}); // Redirect to dashboard page or any other secure page
        });
      })(req, res, next)
      };
    }else{
...
function chargingDebtAmount(maxBalance,lastTimeCharge,userLevel,chargingDebt){
  let lastCharge=Date.now() - lastTimeCharge;
  console.log('153:lastCharge=',lastCharge);
  ...
  if(lastCharge<chargeTime){
    ...
    if(userLevel == 'Gold'){
      return goldUserCharge
    }else if(userLevel == 'Silver'){
      return silverUserCharge
    }else if(userLevel == 'Bronz'){
      return bronzUserBalance
    }else if(userLevel == 'Starter' || userLevel == 'Tester'){
      return starterUserCharge
    }
  }
...
function RemainActiveTimeChecking(userId){
  ...
  if(userInfo.port !== 60000){
    ...
    if(remainActiveTime>0 && userInfo.active == true){
      ...
      execTerminalCommand(userPort,`pm2 stop pm2App${userPort}.json`);
    }
  }
}
