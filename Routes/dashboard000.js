...
async function excutionBot(){
    ...
    router.post('/inputData', async (req, res) => {
        let inputData = {
            ...
            NFBO: parseFloat(req.body.inputData.NFBO),
    ...
    router.put('/editData', async (req,res)=>{
        let editData = {
            ...
            NFBO: parseFloat(req.body.editData.NFBO),
        ...
        if(foundBot){
            ...
            if (botList[indexBot_trading]['delete_buttom']==false) {
                ...
                if (foundOrder) {
                    ...
                    let mathIDPB;
                    let NFBO
                    NFBO=botList[indexBot_trading]['NFBO']
                    if(botList[indexBot_trading]['filled_buy_order']>=NFBO && NFBO>0){
                        NFBO=botList[indexBot_trading]['filled_buy_order']
                    }else{
                        NFBO=1
                    }
                    console.log('840:NFBO=',NFBO)
                    ...
                    if((botList[indexBot_trading]['Deep_dowun_buy'] === true) && (botList[indexBot_trading]['filled_buy_order'] > 0) && (filled_buy_order_coff === 0)){
                        mathIDPB=Math.pow((1+botList[indexBot_trading]['IDPB']/(100*NFBO)),botList[indexBot_trading]['filled_buy_order'])*(botList[indexBot_trading]['filled_buy_order']);
                        console.log('844:mathIDPB=',mathIDPB)
                    }else{
                        mathIDPB=Math.pow((1+botList[indexBot_trading]['IDPB']/(100*NFBO)),botList[indexBot_trading]['filled_buy_order']);
   ...
   // --------------------------------------------------
    ...
    async function buyNew(orderInfo,indexBot_trading){
        ...
        if (botList[indexBot_trading]['softBuy']==true){
            ...
            console.log('844:NFBO=',NFBO)
            mathIDPB=Math.pow((1+botList[indexBot_trading]['IDPB']/(100*NFBO)),botList[indexBot_trading]['filled_buy_order']);
            console.log('893:mathIDPB=',mathIDPB)
            let DPB_coeff=botList[indexBot_trading]['DPB']*mathIDPB;
            let newBuyPrice = (1 - DPB_coeff/ 100) * botList[indexBot_trading]['price'];
            let new_down_BuyPrice = Math.floor(newBuyPrice * botList[indexBot_trading]['roundPricePow']) / botList[indexBot_trading]['roundPricePow'];
            // ---------------------
            ...
            if(data_buy.price<new_down_BuyPrice && botList[indexBot_trading]['SBF']>=0.1 && botList[indexBot_trading]['SDPB']>0 && botList[indexBot_trading]['TBF']>0){
                console.log('716:botList[indexBot_trading].TBF=',botList[indexBot_trading]['TBF']);
