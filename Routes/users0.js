...
const msToTime = require('../Models/msToTime');
const showCurrentTime = require('../Models/showCurrentTime');
...
//global varyables
let trialTestTime=2* 60 * 1000; // 50 minutes in milliseconds for trial testing
let chargeTime=5* 60 * 1000; // 10 minutes in milliseconds for charghing period
let deActivePeriodTime=4* 60 * 1000; // 50 minutes in milliseconds for deactive period
// =========================start internal coding===========================
RemainActiveTimeChecking_inUsers()
// ==============================================
// ***************routings***************
//**************login Page****************
    ...
    if(foundEmail){
      ...
      try {
        ...
        let newUser={
          ...
          chargTimeStatus:showCurrentTime(),
          chargingDebt:1,
          userLevel:'Tester',
          delete_buttom:false,
          stop_buttom:false,
        ...
        setTimeout(function () { RemainActiveTimeChecking_inAdmin(userInfo.id) }, trialTestTime);
...
router.post("/login", (req, res, next) => {
  ...
  if(!email || !password || !authKey){ // check required fields:
    ...
    const auth=userStore.getAllUsers();
    let foundUser=auth.some(user => user.email === email);
    if(foundUser){
      let userInfo=auth.find(user => user.email === email);
      if(userInfo.port !== userInfo.port){ // check Perimission Port:
        console.log('187:userInfo.port=',userInfo.port);
        return res.json({LoginAuthentication: false, msg:'authentication port Error-permission. Please Use correct port.'});
      }else{
        ...
        if((userInfo.key1 != "YAHYABAYAT1364")){
          if((userInfo.userLevel == 'Tester')){
            token = jwt.sign({ wallet: userInfo.id }, JWT_SECRET, { expiresIn: '5m' });
          }
          let chargingStatus=RemainChargTimeChecking(userInfo);
          RemainChargTime=chargingStatus.RemainChargTime;
          chargTimeStatus=chargingStatus.chargTimeStatus;
          console.log('184:chargingDebt=',chargingDebt);
          chargingDebt=chargingDebtAmount(userInfo.maxBalance,userInfo.lastTimeCharge,userInfo.userLevel,userInfo.chargingDebt);
          console.log('185:chargingDebt=',chargingDebt);
          console.log("162:chargTimeStatus=",chargTimeStatus);
          // تغییر یک فیلد:
          // userStore.updateField("chargingDebt", chargingDebt);
          userInfo.chargingDebt=chargingDebt;
          userInfo.chargTimeStatus=chargTimeStatus;
          userStore.addOrUpdateUser(userInfo);
        }else if(userInfo.key1 == "YAHYABAYAT1364"){
          token = jwt.sign({ wallet: userInfo.id }, JWT_SECRET, { expiresIn: '1000m' });
          RemainChargTime=10000;
          chargTimeStatus="adminLogin"
          chargingDebt=0;
        }
        ...
        req.logIn(user, (err) => {
          ...
          console.log("163:chargTimeStatus=",chargTimeStatus);
          return res.json({LoginAuthentication: true,userKey1:userInfo.key1,msg:userInfo.port,RemainChargTime:RemainChargTime,chargTimeStatus:chargTimeStatus, userLevel:userInfo.userLevel, token:token,chargingDebt:chargingDebt}); // Redirect to dashboard page or any other secure page
        });
      })(req, res, next)
      };
    }else{
...
router.post("/AnswersQuestions",(req,res)=>{
  ...
  const userInfo = userStore.getUserInfo();
  const userAnswers=secretData.find(user =>user.port == userInfo.port);
  ...
  res.json({forgetAuthentication:forgetAuthentication, userPort:userInfo.port} )
});
// *****************************
// *************passwordChange*************
...
function pm2AppModify(userPort){
    ...
    newUser.cwd="/Users/Administrator/aays01/7";
    // newUser.log_file="/Users/Administrator/aays01/7/log_files/"+userPort+"/"+userPort+".log"
    // newUser.log_file=path.join(logDir, `${userPort}.log`), // Constructed log file path
    newUser.out_file = "C:\\Users\\Administrator\\aays01\\7\\log_files\\" + userPort + "\\" + userPort + ".log";  // Output log path
    newUser.error_file="/Users/Administrator/aays01/7/log_files/"+userPort+"/"+userPort+"_error.log"
...
// ---------------------------
function execTerminalCommend(userPort){
  let directory = `C:\\Users\\Administrator\\aays01\\7\\StorageData\\${userPort}`;
...
function RemainChargTimeChecking(userInfo){
  ...
  if(RemainChargTime>0){
    chargTimeStatus=msToTime(RemainChargTime);
    return {RemainChargTime:RemainChargTime, chargTimeStatus:chargTimeStatus}
  }else{ 
    remainActiveTime=(chargePeriodTime+deActivePeriodTime)-(Date.now()-parseInt(userInfo.lastTimeCharge));
    console.log('706:remainActiveTime=',remainActiveTime)
    chargTimeStatus=msToTime(remainActiveTime);
    return  {RemainChargTime:0, chargTimeStatus:chargTimeStatus}
  }
}

//=====================================
//تابع میزان بدهی هر کاربر 
function chargingDebtAmount(maxBalance,lastTimeCharge,userLevel,chargingDebt){
  let chargePeriod=5*24*60*60*1000;//period of charge (1 month)
  let chargeTime=Date.now() - lastTimeCharge;
  console.log('153:chargeTime=',chargeTime);
  // let starterUserCharge=1+0.001*maxBalance;;
  // let bronzUserCharge=2+0.001*maxBalance;
  // let silverUserCharge=3+0.001*maxBalance;
  // let goldUserCharge=4+0.001*maxBalance;
  // let bronzUserBalance=1;
  // let silverUserBalance=1.3;
  // let goldUserBalance=1.5;
  let starterUserCharge=1;
  let bronzUserCharge=2;
  let silverUserCharge=3;
  let goldUserCharge=4;
  let bronzUserBalance=2;
  let silverUserBalance=3;
  let goldUserBalance=5;
  if(chargeTime<chargePeriod){
    console.log('154:maxBalance=',maxBalance);
    if(maxBalance>goldUserBalance){
      if(userLevel == 'Gold'){
        return chargingDebt=0
      }else if (userLevel == 'Silver'){
        return chargingDebt=goldUserCharge-silverUserCharge
      }else if (userLevel == 'Boronz'){
        return chargingDebt=goldUserCharge-bronzUserCharge
      }else if ((userLevel == 'Starter') || (userLevel == 'Tester')){
        return chargingDebt=goldUserCharge-starterUserCharge
      }
    }else if(maxBalance>silverUserBalance){
      if(userLevel == 'Gold'){
        return chargingDebt=0
      }else if (userLevel == 'Silver'){
        return chargingDebt=0
      }else if (userLevel == 'Boronz'){
        return chargingDebt=silverUserCharge-bronzUserCharge
      }else if ((userLevel == 'Starter') || (userLevel == 'Tester')){
        return chargingDebt=silverUserCharge-starterUserCharge
      }
    }else if(maxBalance>bronzUserBalance){
      if(userLevel == 'Gold'){
        return chargingDebt=0
      }else if (userLevel == 'Silver'){
        return chargingDebt=0
        ...
        return chargingDebt=bronzUserCharge-starterUserCharge
      }
    }else{
      console.log('155:chargingDebt=',chargingDebt);
      return chargingDebt
    }
  }else{
    return chargingDebt
...
// --------------------------------------------------
//deActivationCheck
async function RemainActiveTimeChecking_inAdmin(userId){
  ...
  if(remainActiveTime>0 && userInfo.active == true){
    setTimeout(function () { RemainActiveTimeChecking_inAdmin(userId) }, chargeTime);
  }else{ 
    userInfo.active=false;
    userInfo.stop_buttom=true;
    userInfo.chargTimeStatus=showCurrentTime(),
    userStore.addOrUpdateUser(userInfo);
    const pm2Command = `pm2 stop pm2App${userInfo.port}.json`;
    console.log('145:pm2Command=',pm2Command)
    await execTerminalCommand(userInfo.port, pm2Command);
  }
}
// -----------------------------------------
async function RemainActiveTimeChecking_inUsers(){
  const userInfo = userStore.getUserInfo();
  if(userInfo.port !== 60000){
    let remainActiveTime
    if(userInfo.userLevel == 'Tester'){
      remainActiveTime=(trialTestTime+deActivePeriodTime)-(Date.now()-parseInt(userInfo.lastTimeCharge))
      console.log('191:remainActiveTime=',Date.now()-parseInt(userInfo.lastTimeCharge))
      console.log('192:remainActiveTime=',trialTestTime+deActivePeriodTime)
      console.log('193:remainActiveTime=',remainActiveTime)
    }else{
      remainActiveTime=(chargeTime+deActivePeriodTime)-(Date.now()-parseInt(userInfo.lastTimeCharge));
      console.log('193:remainActiveTime=',remainActiveTime)
    }
    if(remainActiveTime>0 && userInfo.active == true){
      setTimeout(function () { RemainActiveTimeChecking_inAdmin(userInfo.userId) }, remainActiveTime);
    }else if (userInfo.active == true){ 
      userInfo.active=false;
      userInfo.stop_buttom=true;
      userInfo.chargTimeStatus=showCurrentTime(),
      userStore.addOrUpdateUser(userInfo);
      const pm2Command = `pm2 stop pm2App${userInfo.port}.json`;
      console.log('146:pm2Command=',pm2Command)
      await execTerminalCommand(userInfo.port, pm2Command);
    }
